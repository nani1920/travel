...
module.exports = class UserRepository extends BaseRepository{
    ...
    async find(query, select = null, populate = null,sort= { createdAt: -1 },limit = null){
        return UserModel.find(query).select(select).populate(populate).sort(sort).limit(limit)
    }
    ...
    async createOrUpdateById(id, data) {
        if (id) {
            return UserModel.findByIdAndUpdate(id,{...data}, {new: true}).session(this._session)
        }
        return (await new UserModel(data).save({session: this._session})).generateToken()
    }
